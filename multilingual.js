...
const pages = require("./multilingual.json");
...
async function captureMultipleScreenshots(languaje, device) {
    let phase_url = "";
    switch (languaje) {
        case "ES":
        phase_url = process.env.MULTILINGUAL_URL_ES;
            break;
        case "EN":
            phase_url = process.env.MULTILINGUAL_URL_EN;
        break;
        default:
            phase_url = process.env.MULTILINGUAL_URL_EN;
        break;
    }
    //var cookie = process.env.ISCOOKIESET == true ? process.env.MULTILINGUAL_COOKIE:"";

    let fullpath = process.env.IMAGES_FOLDER;
    let browser = null;
    ...
    try {
        ...
        if(process.env.ISCOOKIESET){
            const cookies = [{
                ...
                'value': 'true',
                'domain': process.env.MULTILINGUAL_COOKIE
            }];
...
async function getSS(page,pages,languaje){
    for (const { id, languaje } of pages) {
        var arreglo = languaje.split("/");
        var name = getName(arreglo);
        await page.goto(phase_url + languaje);
...
async function compareMultiLingual(){
    const pages = require("./multilingual.json");

}
async function placeMultiLingualPages(device) {
    var path_prod = "production";
    var path_stag = "staging";
...
async function init() {
    ...
    await captureMultipleScreenshots("ES", "desktop");
    //await captureMultipleScreenshots("EN", "desktop");
